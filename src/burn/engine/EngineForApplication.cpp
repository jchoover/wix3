...
public: // IBootstrapperEngine
    ...
    )
    {
        ...
        DWORD dwType = 0;
        LONGLONG llValue = 0;
        DWORD dwRemaining = sizeof(LONGLONG);
        ...
        if (wzVariable && *wzVariable && pllValue)
        {
            ...
            if (REG_QWORD != dwType)
            {
                ExitOnFailure(hr = HRESULT_FROM_WIN32(ERROR_INVALID_DATATYPE), "Related bundle %ls shared variable %ls is of an invalid type.", wzBundleId, wzVariable);
            }
            *pllValue = llValue;
...
#ifdef DEBUG
        ::Log(REPORT_STANDARD, "EngineForApp: SetUpdate() - local source: %ls, download source: %ls", wzLocalSource, wzDownloadSource);
#endif
